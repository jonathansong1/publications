...
def run_model(data_pipeline, model, data_source, prefilter = None):
    ...
    new_thresh = evaluate(model, X_test, y_test, best_threshold = None)
    return model, X_test, y_test, new_thresh
...
def calibrate(data_pipeline, model, data_source, prefilter = None):
    ...
    # calibrate pre-fit model on validation data (BJH 2019)
    calibrator = CalibratedClassifierCV(model, cv='prefit')
    calibrator.fit(X_test, y_test)

    # test calibration on testing data (BJH 2020)
...
def main(data_pipeline, model_name = '', bootstrap = False, plot=False):
    ...
    # testing calibration on puf, validation on bjh
    # model = data_pipeline.model
    data_source = 'puf19_lite_v3.csv'
    model = calibrate(data_pipeline, data_pipeline.model, data_source, prefilter = 0)

    data_source = 'puf19_lite_v3.csv'
    model, X_test, y_test, new_threshold = run_model(data_pipeline, model, data_source, prefilter = 0)
    ...
    # previous code
    # data_source = 'puf19_lite_v3.csv'
    # model, X_test, y_test, new_threshold = run_model(data_pipeline, data_pipeline.model, data_source, prefilter = 0)
    # print('PUF19_threshold', data_pipeline.best_threshold)
    # if bootstrap:
    #     bootstrap_cis = bootstrap_stats(model, X_test, y_test, data_pipeline.best_threshold).add_prefix('PUF19_')
    #     # print(bootstrap_cis)
    ...
    # model = calibrate(data_pipeline, model, data_source, prefilter = 50)

    data_source = './bjh/2021_BJH_Labs_Notes.csv'
    model, X_test, y_test, new_threshold = run_model(data_pipeline, model, data_source, prefilter = 50)
